...
export default function ReportPage() {
  ...
  const handleVerify = async () => {
    ...
    if (!geminiApiKey) {
      ...
      toast.error('API key is not valid. Please check your configuration.');
      return;
    }
    ...
    try {
      ...
      const text = await response.text(); // Ensure you await this
      console.log('Response text:', text); // Log the raw response

      // Clean the response text
      const cleanedText = text.replace(/```json|```/g, '').trim(); // Remove unwanted characters
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          <div>
            ...
            <input
              ...
              id="type"
              name="type"
              value={verificationResult?.wasteType || newReport.type} // Use verification result if available
              onChange={handleInputChange}
              required
              className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300"
          ...
          <div>
            ...
            <input
              ...
              id="amount"
              name="amount"
              value={verificationResult?.quantity || newReport.amount} // Use verification result if available
              onChange={handleInputChange}
              required
              className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300"
